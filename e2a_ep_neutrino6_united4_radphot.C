...
map<std::string, double> Eqe_offset;
...
if (fChain == 0) return;
 ...
 double prot_accept_mom_lim;
  ...
  double min_good_mom;
  double max_mom;
  Double_t el_sccc_timediff, sc_cc_delt_cut_sect[nsect]={-2,-5,-8,-8,-2,2}, sc_cc_delt_cut_sect_max[nsect]={15,15,10,15,20,20}, el_cc_nphe;
...
if(en_beam[fbeam_en]>1. && en_beam[fbeam_en]<2.)
 ...
 Eqe_offset["3He"]=en_beam[fbeam_en]-1.149;    //to shift the E reconstruction peak locations to be at 0
Eqe_offset["C12"]=en_beam[fbeam_en]-1.123;

Ecal_offset["3He"]=en_beam[fbeam_en]-1.135;
Ecal_offset["C12"]=en_beam[fbeam_en]-1.129;
...
Eqe_offset["3He"]=en_beam[fbeam_en]-2.239;
Eqe_offset["4He"]=en_beam[fbeam_en]-2.213;
Eqe_offset["C12"]=en_beam[fbeam_en]-2.207;
Eqe_offset["56Fe"]=en_beam[fbeam_en]-2.210;

Ecal_offset["3He"]=en_beam[fbeam_en]-2.236;
Ecal_offset["4He"]=en_beam[fbeam_en]-2.235;
Ecal_offset["C12"]=en_beam[fbeam_en]-2.227;
Ecal_offset["56Fe"]=en_beam[fbeam_en]-2.220;
...
Eqe_offset["3He"]=en_beam[fbeam_en]-4.398;
Eqe_offset["4He"]=en_beam[fbeam_en]-4.370;
Eqe_offset["C12"]=en_beam[fbeam_en]-4.365;
Eqe_offset["56Fe"]=en_beam[fbeam_en]-4.342;

Ecal_offset["3He"]=en_beam[fbeam_en]-4.418;
Ecal_offset["4He"]=en_beam[fbeam_en]-4.415;
Ecal_offset["C12"]=en_beam[fbeam_en]-4.413;
 Ecal_offset["56Fe"]=en_beam[fbeam_en]-4.405;
  ...
  TFile *file_in7 = new TFile("vz_4He_2261_2ndrungroup.root");//vertex correction for 4He 2nd group runs
   TFile *file_out = new TFile(Form("e2a_ep_%s_%s_neutrino6_united4_radphot.root",ftarget.c_str(),fbeam_en.c_str()), "Recreate");
  // TFile *file_out = new TFile(Form("/volatile/clas/clase2/Mariana/Skim_Mariana/e2a_ep_%s_%s_Lucas_withphotons.root",ftarget.c_str(),fbeam_en.c_str()), "Recreate");
...
TH1F *neg_m_ep=new TH1F("neg_m_ep","",300,0,2);
 ...
 if(en_beam[fbeam_en]>1. && en_beam[fbeam_en]<2.){
   ...
   for (int i=0;i<=32;i++)x_qe[i]=-1+i*0.03;
   for (int i=0;i<=22;i++)x_qe[i+33]=-0.04+(i+1)*0.02;
 }
if(en_beam[fbeam_en]>2. && en_beam[fbeam_en]<3.){
   N_qe=55;
   x_qe[N_qe+1];
...
if(en_beam[fbeam_en]>4. && en_beam[fbeam_en]<5.){
   ...
   for (int i=0;i<=16;i++)x_qe[i]=-1+i*0.06;
   for (int i=0;i<=22;i++)x_qe[i+17]=-0.04+(i+1)*0.02;
 }
...
TH2F *h2_ephotangle_ephotphidiff= new TH2F("h2_ephotangle_ephotphidiff","",360,-180,180,300,0,100);
 ...
 n_bins=52;
   ...
   for (int i=0;i<=16;i++)  x_values[i+36]=4.2+(i+1)*0.05;
   }
...
h1_E_rec_43210pi->Sumw2();
 ...
 Long64_t nbytes = 0, nb = 0;
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(n_evnt==1){ SetMomCorrParameters(); SetFiducialCutParameters();}
    n_evnt++;
...
if(en_beam[fbeam_en]>4. && ec[ind_em] > 0.5 &&  sc[ind_em] > 0.5  &&  q[ind_em] <0 && ec[ind_em] >0.5 &&  sc[ind_em] > 0.5 && q[ind_em] <0 && ec_ei[ec[ind_em] - 1] >= 0.055 && ece>=0.33  && p[ind_em]>=min_good_mom) 
 ...
 if( ec[ind_em] < 0.5 ||  sc[ind_em] < 0.5 ||  cc[ind_em] < 0.5 || q[ind_em] >=0) 
  ...
  if(en_beam[fbeam_en]<3.&& en_beam[fbeam_en]>2    &&(ec[ind_em] < 0.5 || cc[ind_em] < 0.5 ||  sc[ind_em] < 0.5 || q[ind_em] >=0 || ec_ei[ec[ind_em] - 1] < 0.06 || ece/p[ind_em]<fsub_e->Eval(p[ind_em]) || ece/p[ind_em] >fsum_e->Eval(p[ind_em]) || p[ind_em]<min_good_mom || cc_c2[cc[ind_em]-1]>=0.1 || el_sccc_timediff<sc_cc_delt_cut_sect[el_cc_sector-1]  || TMath::Sqrt(p[ind_em]*p[ind_em]+e_mass*e_mass)>en_beam[fbeam_en])) //electron pid cuts
    ...
    if(en_beam[fbeam_en]>4. && en_beam[fbeam_en]<5. && el_theta<=16.)  V4_el=V4_el_uncorr;//e- momentum correction at 4 GeV is valid only for el_theta>16
    else    V4_el=EMomentumCorrection(V4_el_uncorr);  
    ...
    double E_rec=(2*(m_prot-eps)*V4_el.E()+m_prot*m_prot-(m_prot-eps)*(m_prot-eps))/(2*(m_prot-eps-V4_el.E()+V4_el.Rho()*cz[ind_em]))+Eqe_offset[ftarget];   //neutrino analysis
    double E_rec_new= (m_prot*eps+m_prot*V4_el.E())/(m_prot-V4_el.E()+V4_el.Rho()*cz[ind_em])+Eqe_offset[ftarget];   //neutrino analysis, Eqe_offset shifts the peak to be located at 0
    E_rec=E_rec_new;
    ...
    if(W_var<2) h2_Q2_nu->Fill(nu,Q2);
    if(W_var<2) h2_Q2_nu_weight->Fill(nu,Q2,1/Mott_cross_sec);
  ...
  for(int k=1;k<=6;k++){
	      ...
	      h2_pimi_theta_p[sc_sect[sc[ind_p]-1]-1]->Fill(p[i],TMath::ACos(cz[i])*TMath::RadToDeg());
	     

	      if(PimiFiducialCut(V3_pimi, &pimi_phimin, &pimi_phimax)){

		h2_pimi_theta_p_cut[sc_sect[sc[ind_p]-1]-1]->Fill(p[i],TMath::ACos(cz[i])*TMath::RadToDeg());
...
for(int k=1;k<=6;k++){
	    ...
	    h2_pipl_theta_p[sc_sect[sc[ind_p]-1]-1]->Fill(p[i],TMath::ACos(cz[i])*TMath::RadToDeg());
	    
	      if (PiplFiducialCut(V3_pipl, &cphil, &cphir)){

		h2_pipl_theta_p_cut[sc_sect[sc[ind_p]-1]-1]->Fill(p[i],TMath::ACos(cz[i])*TMath::RadToDeg());
   ...
   for(int k=1;k<=6;k++){
	...
	if(ec[i] > 0 && dc[i]<=0  && sc[i]<=0  && stat[i] > 0 &&  q[i] == 0)
	  {
	    ...
	    if(neut_beta_corr >EC_photon_beta[ftarget]){   //photon identification
	      ...
	      num_pi_phot=num_pi_phot+1;
	      ind_pi_phot[num_pi_phot - 1]=i;
	      if(V3_phot_angles.Angle(V3_el)*TMath::RadToDeg()<phot_rad_cut && abs(neutr_phi_mod-el_phi_mod)<phot_e_phidiffcut) ec_radstat_n[num_pi_phot - 1]=true;//select radiation photons
    ...
    int ind_prot1,ind_prot2;
    TVector3 V3_q;
    double rot_angle,P_N1, P_N2;
   ...
   if(num_p==1){
	...
	if(q[i]>0 &&  sc[i] > 0 && dc[i]>0 && stat[i] > 0 ){
	  ...
	  pipl_delt_p_ep->Fill(p[i],deltt);
	  pos_m_ep->Fill(TMath::Sqrt(p[i]*p[i]/(b[i]*b[i])-p[i]*p[i]));

	  if (PiplFiducialCut(V3_pipl, &cphil, &cphir))	  pipl_delt_p_epfidcut->Fill(p[i],deltt);
	}
	...
	if(q[i]<0 &&  sc[i] > 0 && dc[i]>0 && stat[i] > 0 ){
	  ...
	  pimi_delt_p_ep->Fill(p[i],deltt);
	  neg_m_ep->Fill(TMath::Sqrt(p[i]*p[i]/(b[i]*b[i])-p[i]*p[i]));

	  if(PimiFiducialCut(V3_pimi, &pimi_phimin, &pimi_phimax))    pimi_delt_p_epfidcut->Fill(p[i],deltt);
	}
...
// <A HREF="http://einstein.unh.edu/protopop/MomentumCorrections/emc4E2.html">Momentum Corrections</A> -- D.Protopopescu(UNH)
 ...
 if( en_beam[fbeam_E]>4. &&  en_beam[fbeam_E]<5.){
   ...
   sprintf(ParLocation,".");// where do we put the parameter file ??
   //
 ...
 else printf("There are no e- momentum corrections parameters to be read at %3.1f GeV!\n",  en_beam[fbeam_E]); 
}
...
Float_t theta =  57.29578*(V3el.Theta()); // (DP 9/12/2000) Needed for my function
 ...
 // Electron momentum correction for 1.1GeV data (1500A and 750A torus currents)
  if((en_beam[fbeam_E]>1) && (en_beam[fbeam_E]<1.2)  && fTorusCurrent>740  && fTorusCurrent<1510) {

    phi -= 60.*sectInd;
    // Momentum correction for 750A torus current
	...
	for(Int_t eachthetapar=0;eachthetapar<4;eachthetapar++) {
	  phipar[eachphipar]+=fgPar_1Gev_750[sectInd][eachphipar][eachthetapar]*pow(theta,eachthetapar);
	}
      }
    ...
    // Momentum correction for 1500A torus current
    if((fTorusCurrent>1490) && (fTorusCurrent<1510)) {
      Double_t phipar[6];
      for(Int_t eachphipar=0;eachphipar<6;eachphipar++) {
	phipar[eachphipar]=0;
	for(Int_t eachthetapar=0;eachthetapar<4;eachthetapar++) {
	  phipar[eachphipar]+=fgPar_1Gev_1500[sectInd][eachphipar][eachthetapar]*pow(theta,eachthetapar);
	}
      }
      Double_t correction=0;
 ...
 if(en_beam[fbeam_E]>4. && en_beam[fbeam_E]<5. && fTorusCurrent>2240 && fTorusCurrent<2260){
    //
    //the correction for 4.4Gev and 2250A data -- by protopop@jlab.org
    //the corrections are valid only for theta > 16. 
    //
    p *= ((fgPar_4Gev_2250_Phi[sectInd][0] + fgPar_4Gev_2250_Phi[sectInd][1]*phi 
	  + fgPar_4Gev_2250_Phi[sectInd][2]*phi*phi)*(fgPar_4Gev_2250_Theta[sectInd][3]
	  + (fgPar_4Gev_2250_Theta[sectInd][2]*theta*theta + fgPar_4Gev_2250_Theta[sectInd][1]*theta 
	  + fgPar_4Gev_2250_Theta[sectInd][0])/exp(theta)));
  }
