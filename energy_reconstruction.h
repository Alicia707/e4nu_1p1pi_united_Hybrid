...
// Wed Jan  2 11:00:43 2019 by ROOT version 6.14/04
// from TChain ch/energy_reconstruction
//////////////////////////////////////////////////////////
...
public :
   ...
   std::string ftarget;
   std::string fbeam_en;
   //Included filter_selection capabilities 9.16.20 - AM//
   std::string ffilter_selection;
// Fixed size dimensions of array or collections stored in the TTree if any.
//Leaf and Branch Types for 'ch' tree 9.17.20 - AM//
...
//Leaf and branch types for 'gst' tree 9.17.20 -AM//
// Declaration of leaf types
  Int_t           iev;
  Int_t           neu;
  Int_t           fspl;
  ...
  Bool_t          nuel;
  Bool_t          em;
  //Bool_t is not used in any of programs, commented out to test 9.17.20 -AM //
//  Bool_t          cc;
  Bool_t          nc;
  Bool_t          charm;
  Int_t           neut_code;
  ...
  TBranch        *b_calresp0;   //!
   ...
   energy_reconstruction(std::string, std::string, std::string, TTree *tree=0);
   virtual ~energy_reconstruction();
...
#ifdef energy_reconstruction_cxx
energy_reconstruction::energy_reconstruction(std::string a_target, std::string a_beam_en, std::string a_filter_selection, TTree *tree) : fChain(0)
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
     ...
     fbeam_en = a_beam_en;
     //9.16.20 -AM//
     ffilter_selection = a_filter_selection;
      ...
      //Changed according to filter_selection implementation 9.16.20 -AM//
      if(ffilter_selection=="unfiltered") f->GetObject("ch",tree);
      else if(ffilter_selection=="filtered") f->GetObject("gst",tree);

#else // SINGLE_TREE

      // The following code should be used if you want this class to access a chain
      // of trees.
      std::cout<<ftarget.c_str() <<"    beam E"<< fbeam_en <<std::endl;
      //9.16.20 -AM//
      if(ffilter_selection=="unfiltered")
      {
        TChain * chain = new TChain("ch", "energy_reconstruction");
        //chain->Add(Form("/work/clas/clase2/Mariana/data/e2a_%s_%s_v1/*.root/h10",ftarget.c_str(),fbeam_en.c_str()));
         chain->Add(Form("/mnt/c/Users/alici/Documents/TraceyCodes/01-18-19/%s_%s_skim.root/h10",ftarget.c_str(),fbeam_en.c_str()));
        tree=chain;
      }
      else if(ffilter_selection == "filtered"){
        TChain * chain = new TChain("gst","energy_reconstruction");
...
Int_t energy_reconstruction::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t energy_reconstruction::LoadTree(Long64_t entry)
{
...
void energy_reconstruction::Init(TTree *tree, std::string a_filter_selection)
{
   ...
   a_filter_selection=ffilter_selection;
// Tests to check the filter selection and sets branch addresses appropriately 9.17.20 -AM //
   if(ffilter_selection=="unfiltered"){
...
Bool_t energy_reconstruction::Notify()
{
...
void energy_reconstruction::Show(Long64_t entry)
{
...
Int_t energy_reconstruction::Cut(Long64_t entry)
{
...
#endif // #ifdef energy_reconstruction_cxx
...
//sector 5 has 3 gaps
//const Double_t fid_1gev_750_efid_S5[3][2][4]={{{111.088,7.4495,-0.810668,0.0314985},{119.088, 7.4495, -0.810668, 0.0314985}},
//					      {{19.8153 , 0.300066 , 1.52498  ,-0.291616},{17.2434,  9.64878 , -4.82847,  0.992597}},
//					      {{34.866 , -19.2535 , 15.4707 , -3.40783},{23.3154 , 10.4615 , -5.5858 , 1.52441}}};

//const Double_t fid_1gev_750_efid_S5[3][2][4]={{{111.088 , 7.4495 , -0.810668 , 0.0314985},{119.088 , 7.4495  ,-0.810668 , 0.0314985}},
//					      {{19.7064 , 0.595424 , 1.22134 , -0.234762},{15.5583 , 12.9437 , -6.95756 , 1.41721}},
...
//sector 4 has 2 gaps
const Double_t fid_1gev_750_efid_S4[2][2][4]={{{30.5039 , 5.98362 , -12.825 , 7.04185},{25.4698 , 13.3482 , -7.32756 ,1.61262}},
					      {{99.758,  1.35143  ,-0.115901,  0.0033244},{111.575 , 0.00204503 , 0.122899 , -0.00687251}}};
//sector 3 has 4 gaps
const Double_t fid_1gev_750_efid_S3[4][2][4]={{{17.1438 , 5.82672 , -1.33914 , 0.370319},{8.71859 , 23.4486 , -10.8172 , 1.97916}},
					      {{37.5065 , 1.33304 , 0.319901 , 0.181076},{38.9433 , 4.26507 , 0.260988 , -0.296786}},
...
const Double_t fid_2gev_2250_pimifid_outline[6][6][5]={ { {437.992 , -6758.33 , 44725 , -130667 , 140000},{ -108.89 , 2441.34 , -16273.9,  46629.1 , -48613.3},{23.4972 , -22.8102 , 58.051 , 455.812 , -1355.76},{-1282.61 , 26039.3 , -176254 , 497349 , -504262},{79.2767 , 10.9846 , -1097.42 , -2505.95 , 11295.1},{-116.656 , 5464.17 , -39666.7 , 121333 , -133333} },
                                                        { {294.906 , -3993.33 , 25700 , -74666.7 , 80000},{-1.73619 , 230.33 , -230.96 , -2041.14 , 4247.97},{-8.35081 , 294.696 , -351.216 , -2783.33 , 6284.44},{ -654.396 , 13447.8 , -95677.4 , 289413 , -316333},{-1087.77 , 21543.4 , -150270 , 446839 , -481346},{-12.4219 , 3796.67 , -30450 , 101333 , -120000} },
                                                        { {169.867 , -1621.67 , 9791.67 , -29333.3 , 33333.3},{1.31563 , 228.273 , -277.831 , -2151.54 , 4681.77},{4.99614 , 143.415 , -99.2912 , -1182.49 , 2131.09},{-442.257 , 10171.2 , -74092.5 , 222973 , -240250},{286.575 , -4807.23 , 31987.4 , -94543 , 102888},{-80.793 , 4620.91 , -32275.5 , 94668.1 , -100002} },
                                                        { {171.781 , -1604.17 , 9133.31 , -25333.3 , 26666.7},{15.4017 , 65.7045 , -29.3075 , -398.534 , 391.892},{-105.713 , 2167.31 , -13426.9 , 36592.6 , -36900.5},{-117.86 , 4749.21 , -44679.4 , 161213 , -199605},{-1228.95 , 24386.5 , -171732 , 516890 , -564141},{314.227 , -2770.83 , 17208.3 , -46666.6 , 46666.7} },
		                                        { {227.727 , -2592.5 , 15608.3 , -44000 , 46666.7},{-108.819 , 2413.21 , -16243.6 , 47773.3 , -51709},{-1.34202 , 206.441 , -194.54 , -1788.54 , 3644.16},{-2105.78 , 42101.1 , -299023 , 908108 , -1.00031e+06},{-542.533 , 11447.1 , -83967.3 , 260870 , -292292},{198.746 , -468.327 , 479.183 , 5155.48 , -11578.4} },
	                                                { {205.403 , -1966.22 , 9777.77 , -22675.4 , 20010.2},{5.0732 , 177.775 , -195.135 , -1565.26 , 3219.17},{114.9 , -2022.82 , 15044.2 , -46493.2 , 51247.4},{214.243 , -4966.33 , 42750.9 , -149352 , 180657},{1230.36 , -24868.8 , 180507 , -555189 , 613732},{166.471 , -85.4706 , -94.7685 , 1150.18 , -2073.65} } };
...
const Double_t fid_2gev_2250_efid_S5[2][2][4]={{{88.912, 39.7076 ,-11.7886, 1.219},{102.112 ,37.1564 ,-12.6383 ,1.49529}},
					      {{100.903 ,-0.978802 ,1.99633 ,-0.285675},{103.959, 2.03364, -0.0566607 ,0.11236}}};

//sector 3 has 2 gaps
const Double_t fid_2gev_2250_efid_S3[2][2][4]={{{112.584, -8.50953 ,3.65541 ,-0.321358},{ 99.7915 ,11.1871 ,-1.96114 ,0.167723}},
					       {{15.4065, 35.6826 ,-11.4842 ,1.35174},{-8.2081, 44.1171 ,-9.23191 ,0.696626}}};

//sector 1 has 2 gaps
const Double_t fid_2gev_2250_efid_S1[2][2][4]={{{113.956, 0.363286, 0.198558 ,-0.0144791},{ 124.963 ,-1.25147 ,0.390258, -0.0194753}},
					       {{ 172.863 ,-31.0937 ,6.70311, -0.4403},{ 135.614 ,-2.55523 ,0.881528 , -0.0632905}}};

//sector 4 has 1 gap
const Double_t fid_2gev_2250_efid_S4[2][4]={{99.1183 ,3.87431 ,-0.651923, 0.0541116},{114.798, -1.74102, 0.670956, -0.0506822}};

//sector 6 has 1 gap
const Double_t fid_2gev_2250_efid_S6[2][4]={{80.5638, -15.6315 ,3.64252 ,-0.267854},{ 51.8427, 10.8915,-2.38221, 0.177976}};
